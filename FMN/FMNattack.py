...
from Utils.losses import difference_of_logits
...
def fmn(model: nn.Module,
    ...
    if norm == 0:
        epsilon = torch.ones(batch_size, device=device) if starting_points is None else delta.flatten(1).norm(p=0,
                                                                                                              dim=0)
    else:
    ...
    for i in range(steps):
        ...
        if norm == 0:
            ...
            distance_to_boundary = loss.detach().abs() / delta.grad.data.flatten(1).norm(p=dual, dim=1).clamp_(
                min=1e-12)
            epsilon = torch.where(is_adv,
