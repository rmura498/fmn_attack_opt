        ...
        for dataset_id, dataset_name in enumerate(datasets):
            ...
            for opt in OPTIMIZERS:
                for sch in SCHEDULERS:
                    tuning_cmd = f'python tune_pbt.py --model_id {model_id} --dataset_id {dataset_id} --optimizer {opt} --scheduler {sch} --batch {batch} --steps {steps} --num_samples {num_s} --epochs {epochs} --dataset_percent {dt_percent} --working_path {tuning_exp_wp}\n'
